...
namespace detail{
	...
	inline image loadDDS(std::string const & Filename)
	{
		...
		std::size_t Curr = FileIn.tellg();
		FileIn.seekg(0, std::ios_base::end);
		std::size_t End = FileIn.tellg();
		FileIn.seekg(Curr, std::ios_base::beg);

		std::vector<glm::byte> Data(End - Curr, 0);
		std::size_t Offset = 0;
		...
		for(std::size_t Level = 0; Level < Image.levels() && (Width || Height); ++Level)
		{
			...
			Image[Level] = mipmap(Width, Height, Format, MipmapData);
	...
	inline image loadTGA(std::string const & Filename)
	{
		...
		mipmap Mipmap(Width, Height, Format);
	...
	inline void saveTGA(gli::image const & ImageIn, std::string const & Filename)
	{
		...
		unsigned short Width = Image[0].width();
		unsigned short Height = Image[0].height();
		unsigned char TexelSize = Image[0].value_size() << 3;
		unsigned char Descriptor = 0;

		if(TexelSize == 24)
			Image.swizzle<glm::u8vec3>(glm::R, glm::G, glm::B, glm::A);
		if(TexelSize == 32)
			Image.swizzle<glm::u8vec4>(glm::R, glm::G, glm::B, glm::A);
		...
		delete[] IdentificationField;
		FileOut.write((char*)Image[0].data(), Image[0].size());
		if(FileOut.fail() || FileOut.bad())
			return;
...
inline image loadImage(std::string const & Filename)
{
...
inline void saveImage(image const & Image, std::string const & Filename)
{
